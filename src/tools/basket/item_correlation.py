        ...
        with get_db() as db:
            ...
            if not data:
                return format_response(
                    success=True, data=[], debug_sql=query,
                    metadata={'target_item': {'id': item_id}, 'message': 'No correlations found'})
            target_info = {
                'id': item_id,
            ...
            for row in data:
                correlations.append({
                    ...
                    'purchase_behavior': {
                        'avg_qty_together': round(float(row['avg_qty_together']), 2),
                        'avg_sales_together': round(float(row['avg_sales_together']), 2),
                        'total_qty_together': round(float(row['total_qty_together']), 2),
                        'total_sales_together': round(float(row['total_sales_together']), 2)
                    }
                })
            return format_response(
                success=True, data=correlations, debug_sql=query,
                metadata={'target_item': target_info,
                          'parameters': {'date_range': f"{start_date} to {end_date}",
                                         'min_frequency': min_frequency,
                                         'site_id': site_id}})
...
item_correlation_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "min_frequency": {"type": "integer", "description": "Minimum co-occurrence count"},
