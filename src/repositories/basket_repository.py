...
class BasketRepository(BaseRepository[SalesFact]):
    ...
    def basket_analysis(
        ...
        # Base filters
        base_filters = [item1.SaleDate >= start_date, item1.SaleDate <= end_date]
        if site_id:
            base_filters.append(item1.SiteID == site_id)

        # Count total transactions
        total_transactions = (
            self.session.query(func.count(func.distinct(SalesFact.TransactionID)))
        ...
        pairs_query = (
            ...
            .filter(and_(*base_filters))
            .group_by(item1.ItemID, item1.ItemName, item2.ItemID, item2.ItemName)
        )
        ...
        for pair in pairs_query.all():
            ...
            lift = (
                (pair.pair_count * total_transactions) / (item1_count * item2_count)
                if item1_count > 0 and item2_count > 0
                else 0
            )
    ...
    def item_correlation(
        ...
        target_item = (
            self.session.query(SalesFact.ItemName, SalesFact.Category)
            .filter(SalesFact.ItemID == item_id)
            .first()
        )
        ...
        target_transactions = self.session.query(SalesFact.TransactionID).filter(
            and_(
        ...
        if site_id:
            target_transactions = target_transactions.filter(
                SalesFact.SiteID == site_id
            )
        ...
        correlated = (
            correlated.group_by(
                SalesFact.ItemID, SalesFact.ItemName, SalesFact.Category
            )
            .having(func.count(func.distinct(SalesFact.TransactionID)) >= min_frequency)
    ...
    def basket_metrics(
            ...
            for group in group_by:
                if group == "site":
                    basket_subquery = basket_subquery.add_columns(
                        SalesFact.SiteID, SalesFact.SiteName
                    )
        ...
        basket_subquery = basket_subquery.filter(
            and_(SalesFact.SaleDate >= start_date, SalesFact.SaleDate <= end_date)
        )
        ...
        if group_columns:
            ...
            metrics_query = metrics_query.group_by(
                *[basket_data.c[col] for col in group_columns]
            )
    ...
    def _get_item_frequency(
        self, item_id: int, start_date: date, end_date: date, site_id: Optional[int]
    ) -> int:
